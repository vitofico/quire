 ...
 * #11 amendment (stdout-only audit; NO ai_generation_log row in PR-ζ)
...
async def test_promote_emits_stdout_audit_no_db_row(
    client_factory, configure_ai, app, session, caplog
):
    """Lock #11 amendment: stdout audit ONLY; no ai_generation_log row."""
    async with client_factory(ai_enabled=True, ai_base_url="http://x", ai_model="m") as client:
        configure_ai(app, {"schema_version": 4, "intro": "ok", "confidence": "low"})
        await _opt_in(client, "alice")
        await _seed_source_insight(
        ...
        # Bytes generated by opt-in PUT etc may emit their own logs; snapshot
        # the AIGenerationLog set BEFORE promote and ensure it doesn't grow.
        pre = (await session.execute(select(AIGenerationLog.id))).scalars().all()
    ...
    assert r.status_code == 200
    # Stdout audit line present.
    msgs = [rec.getMessage() for rec in caplog.records]
    assert any("event=ai.promote" in m and "outcome=copied" in m for m in msgs), msgs
    # NO new ai_generation_log row from the promote call.
    post = (await session.execute(select(AIGenerationLog.id))).scalars().all()
